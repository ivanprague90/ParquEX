...
public class AddQuestionController extends MainController {
	...
	private void handleButtonConfirm(ActionEvent event) throws IOException {
		boolean existValidAnswer = false;

		for (int i = 0; i < validAnswerList.size(); ++i) {
			if (!validAnswerList.get(i).getText().isEmpty()) {
				existValidAnswer = true;
				break;
			}
		}

		if (!txtModifyAttribute.getText().isEmpty()
				&& !txtModifyQuestion.getText().isEmpty() && existValidAnswer) {
			boolean existingAttribute = false;

			for (Map.Entry<String, QuestionTO> entry : Question.getQuestions()
					.entrySet())
				if (entry.getValue().getAttribute()
						.equals(txtModifyAttribute.getText()))
			...
			if (!existingAttribute) {
				...
				for (int i = 0; i < numPrecursor; ++i) {
					...
					if (choiceAttributePrecursorList.get(i).getSelectionModel()
							.getSelectedItem() == null)
						continue;
					prec.setIdQuestion(choiceAttributePrecursorList.get(i)
							.getSelectionModel().getSelectedItem().getId());
					if (switchIsOrNotList.get(i).switchOnProperty().getValue())
						prec.setIsOrNot("is");
					else
						prec.setIsOrNot("is-not");
					if (choiceAnswerPrecursorList.get(i).getSelectionModel()
							.getSelectedItem() == null)
						continue;
					prec.setAnswer(choiceAnswerPrecursorList.get(i)
				...
				lblNotifyModQuestion.setWrapText(true);
				lblNotifyModQuestion
						.setText("Attributo già esistente");
			}				
		} else {
			lblNotifyModQuestion.setWrapText(true);
			lblNotifyModQuestion
					.setText("Attributo, Domanda e Risposte valide non possono essere vuoti");
		}
